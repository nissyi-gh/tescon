...
module Tescon
  module Rules
    # Finds RSpec match expectations that can be translated to minitest assertions.
    class ExpectMatch
      ...
      class Visitor < Prism::Visitor
        ...
        def finding_for(node)
          ...
          match_arg = matcher(node).arguments.arguments.first

          Finding.new(
            rule_name: RULE_NAME,
            message: "Convert RSpec match expectation to minitest match assertion",
            start_offset: node.location.start_offset,
            end_offset: node.location.end_offset,
            replacement: "expect(#{expect_arg.location.slice}).#{expectation_method} #{match_arg.location.slice}"
          )
        end

        def expectation_method_for(node)
          return unless expect_match_call?(node)

          { to: "must_match", not_to: "wont_match" }[node.name]
        end

        def expect_match_call?(node)
          %i[to not_to].include?(node.name) &&
            expect_call?(node.receiver) &&
            match_matcher?(matcher(node))
        end
