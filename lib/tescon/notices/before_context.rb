...
module Tescon
  module Notices
    # Flags RSpec before(:context) / after(:context) for minitest-spec migration.
    class BeforeContext
      RULE_NAME = "before_context"
      ...
      class Visitor < Prism::Visitor
        ...
        def notice_for(node)
          return unless context_hook?(node)

          Notice.new(
            rule_name: RULE_NAME,
            line: line_for(node),
            severity: :todo,
            message: "#{node.name}(:context) is RSpec-specific; consider #{node.name}(:all) for minitest-spec"
          )
        end

        def context_hook?(node)
          HOOKS.include?(node.name) &&
            node.receiver.nil? &&
            node.block &&
            symbol_argument(node)&.unescaped == "context"
        end
