...
`spec/rails_helper.rb` などで **spec より先に** 読み込みます（test-prof の `let_it_be` より前に hook を登録するため）:

```ruby
require "tescon/trace"
```

`let_it_be` / `before_all`（`before(:context)`）内の factory 呼び出しも記録されます。各 `context` ごとに `role: before_context` の setup example が生成され、通常の `it` には祖先 setup への参照が `inherited_setup` として付きます。factory 呼び出しのない空 setup は YAML 出力時に除外されます。

RSpec suite 終了時に `tmp/tescon/provenance/` 配下へ、spec ファイル単位で YAML を書き出します。出力先ディレクトリは環境変数で変更できます:

```bash
TESCON_TRACE_PATH=tmp/tescon/provenance bundle exec rspec
```

デフォルトでは `attributes` から `id` / `created_at` / `updated_at` を除外します。フルダンプが必要な場合:

```bash
TESCON_TRACE_FULL_ATTRIBUTES=1 bundle exec rspec
```

各 YAML には `meta` ブロック（`schema_version`, `source_spec`, `generated_at` など）と `examples` が含まれます。タイムスタンプは ISO 8601 文字列に正規化されます。
...
- `let_it_be` や `before_all` 内の factory 呼び出し → `role: before_context` の setup example に記録
- 各 `it` には祖先 setup への参照が `inherited_setup` として付与（fixture 変換時は `inherited_setup` + 自身の `factory_calls` を参照）
- factory 呼び出しのない空 setup example は YAML 出力時に除外
